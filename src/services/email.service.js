...
const sendEmail = async ({ to, subject, htmlContent }) => {
  ...
  try {
    ...
    console.error(`\n❌ [BREVO ERROR] Failed to send email to ${to}:`, error.response?.data || error.message);
    console.log(`\n📢 [FALLBACK] Email Content for ${to}:\nSubject: ${subject}\nBody: ${htmlContent.replace(/<[^>]*>/g, '').trim()}\n`);
  }
};
    ...
    htmlContent: `
      <div style="font-family:Inter,Arial,sans-serif;max-width:520px;margin:auto;background:#0f172a;color:#f1f5f9;border-radius:16px;overflow:hidden">
        ...
        <div style="padding:32px">
          ...
          <p style="color:#64748b;font-size:13px">If you didn't request this, secure your account immediately.</p>
        </div>
      </div>
    `,
  });
